...
public class QuadTree<T> where T : class
{
    ...
    private readonly Rect bounds; // overall bounds we are indexing.
    ...
    {
        if (this.bounds.Width == 0 || this.bounds.Height == 0) throw new ArgumentException("Bounds must be non zero");
    ...
    internal class Quadrant
    {
        ...
        private readonly Rect bounds; // quadrant bounds.
        ...
        internal void GetIntersectingNodes(List<QuadNode> nodes, Circle bounds, bool doNotCheck = false)
        {
            ...
            // See if any child quadrants completely contain this node.
            if ((doNotCheck || bounds.IntersectsWith(new Rect(this.bounds.Left, this.bounds.Top, w, h))) &&
                topLeft != null)
                topLeft.GetIntersectingNodes(nodes, bounds, doNotCheck);

            if ((doNotCheck || bounds.IntersectsWith(new Rect(this.bounds.Left + w, this.bounds.Top, w, h))) &&
                topRight != null)
                topRight.GetIntersectingNodes(nodes, bounds, doNotCheck);

            if ((doNotCheck || bounds.IntersectsWith(new Rect(this.bounds.Left, this.bounds.Top + h, w, h))) &&
                bottomLeft != null)
                bottomLeft.GetIntersectingNodes(nodes, bounds, doNotCheck);

            if ((doNotCheck || bounds.IntersectsWith(new Rect(this.bounds.Left + w, this.bounds.Top + h, w, h))) &&
                bottomRight != null)
